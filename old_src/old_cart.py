...
class Cart:
    ...
    def steer(self, speed, angle):
        speed = int(speed) if speed else int(self.velocity)
        ...
        scale = 1
        if angle < 0:
            leftwheel = int((1 + angle * scale) * speed)
        if angle > 0:
            rightwheel = int((1 - angle * scale) * speed)
        # leftwheel_back=int(leftwheel*1.1)
        # rightwheel_back=int(rightwheel*1.1)
        # leftwheel, rightwheel = rightwheel, leftwheel
        # print('angle = {}, leftwheel = {}, rightwheel = {}'.format(angle, leftwheel, rightwheel))  # DEBUG
